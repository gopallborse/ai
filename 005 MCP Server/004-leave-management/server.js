...
function initDatabase() {
  ...
  if (employeeCount.count === 0) {
    ...
    const insertManyRequests = db.transaction((rows) => {
...
function loadLeaveRequests() {
  const db = getDb();
  const rows = db.prepare("SELECT * FROM leave_requests").all();
  db.close();
  return rows;
}
...
function findSimilarEmployees(name, threshold = 0.6) {
...
function formatLeaveRequest(req) {
  let result = "";
...
mcp.registerResource(
  "employee-info",
  new ResourceTemplate("employee://{employee_id}", {
    list: undefined,
  }),
  {
    ...
    if (rows.length === 0) {
      return {
        contents: [
          {
            uri: uri.href,
            text: `No ${status} leave requests found`,
          },
        ],
      };
    }
  ...
  async ({
    ...
    try {
      ...
      const nextNum = lastId
        ? Number.parseInt(lastId.request_id.slice(3), 10) + 1
        : 1;
...
mcp.registerTool(
  ...
  {
    description:
      "Approve a pending leave request and update the employee leave balance",
    inputSchema: z.object({
      request_id: z.string(),
  ...
  async () => {
    ...
    try {
      ...
      const totalRequests = db
        .prepare("SELECT COUNT(*) AS count FROM leave_requests")
        .get().count;
      ...
      const deniedRequests = db
        .prepare(
          "SELECT COUNT(*) AS count FROM leave_requests WHERE status = 'denied'",
        )
        .get().count;
...
mcp.registerTool(
  ...
  async ({
    ...
    try {
      ...
      const nextNum = lastId
        ? Number.parseInt(lastId.employee_id.slice(3), 10) + 1
        : 1;
